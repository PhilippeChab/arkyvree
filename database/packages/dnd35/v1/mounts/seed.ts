...
import {
  SPECIAL_MOUNT_APTITUDES,
  SPECIAL_MOUNT_CLASS,
  SPECIAL_MOUNT_CLASS_FEATURE_FEATS,
  SPECIAL_MOUNT_RACE_PICK_FEATS,
  SPECIAL_MOUNT_RACES,
} from "@/database/packages/dnd35/v1/mounts/data.ts";
...
export default async function seed(db: Db) {
  ...
  // Same seed is called from v1 (fresh installs) and from v47 (prod upgrade
  // path). Skip if already applied.
  const existing = await db
    .select({ id: klassesInRules.id })
    .from(klassesInRules)
    .where(and(eq(klassesInRules.rulesetId, ruleset.id), eq(klassesInRules.kind, "mount")))
    .limit(1);
  if (existing.length > 0) return;

  await db.insert(aptitudesInRules).values(
    SPECIAL_MOUNT_APTITUDES.map((name) => ({ rulesetId: ruleset.id, name })),
  );
  ...
  await seedFeats(db, ruleset.id, aptMap, SPECIAL_MOUNT_CLASS_FEATURE_FEATS);
  await seedFeats(db, ruleset.id, aptMap, SPECIAL_MOUNT_RACE_PICK_FEATS);

  const insertedRaces = await db.insert(racesInRules).values(
    SPECIAL_MOUNT_RACES.map((r) => ({
  ...
  for (const race of SPECIAL_MOUNT_RACES) {
  ...
  await seedClass(db, ruleset.id, SPECIAL_MOUNT_CLASS, {
    rulesetId: ruleset.id,

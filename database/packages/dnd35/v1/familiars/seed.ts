...
import {
  FAMILIAR_APTITUDES,
  FAMILIAR_CLASS,
  FAMILIAR_CLASS_FEATURE_FEATS,
  FAMILIAR_RACE_PICK_FEATS,
  FAMILIAR_RACES,
} from "@/database/packages/dnd35/v1/familiars/data.ts";
...
export default async function seed(db: Db) {
  ...
  // Same seed is called from v1 (fresh installs) and from v45 (prod upgrade
  // path). Skip if already applied.
  const existing = await db
    .select({ id: klassesInRules.id })
    .from(klassesInRules)
    .where(and(eq(klassesInRules.rulesetId, ruleset.id), eq(klassesInRules.kind, "familiar")))
    .limit(1);
  if (existing.length > 0) return;

  await db.insert(aptitudesInRules).values(
    FAMILIAR_APTITUDES.map((name) => ({ rulesetId: ruleset.id, name })),
  );
  ...
  await seedFeats(db, ruleset.id, aptMap, FAMILIAR_CLASS_FEATURE_FEATS);
  await seedFeats(db, ruleset.id, aptMap, FAMILIAR_RACE_PICK_FEATS);

  const insertedRaces = await db.insert(racesInRules).values(
    FAMILIAR_RACES.map((r) => ({
      rulesetId: ruleset.id,
      name: r.name,
      description: r.description,
      size: r.size,
      baseSpeed: r.baseSpeed,
      kind: r.kind ?? "familiar",
    })),
  ...
  for (const race of FAMILIAR_RACES) {
  ...
  await seedClass(db, ruleset.id, FAMILIAR_CLASS, {
    rulesetId: ruleset.id,

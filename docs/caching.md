...
Ownership resolution sometimes needs the stored IDs. Use `Modifiers.findStoredOne(db, { id })` for that case: its shared-repository registration opts out of input and output COW remapping for this method only. It still participates in request deduplication, while ordinary `Modifiers.findOne` reads retain automatic resolution. Keep the SQL in the repository.
